...
export default function SlugPage({ params }: { params: { slug: string, product_category: string, subcategory: string } }) {
  const { slug, product_category, subcategory } = params
  ...
  if (slug in category_products && product_category in (category_products as any)[slug]) {
    ...
    _subcategory = (_product as CategoryItem).items.filter(e => e.href === subcategory)[0]
    if (!_subcategory) {
      notFound()
    }
  } else {
  ...
  const sliderHitItems = _product.items.map((item: SubCategoryItem, i: number) => (
    item.products.slice(0, 6).map((el: ProductItem, i: number) => (
      <div key={i} className='mx-2'>
        <ProductCard
          href={`/${slug}/${product_category}/${subcategory}/${el.id}`}
    ...
    <div className='container'>
      ...
      <div className='grid grid-cols-4 w-full pt-5 pb-10'>
        <div className='col-span-1 mr-5'>
          <h2 className='text-header-XL text-dark font-medium'>{_subcategory.title}</h2>
          <Filters className="mt-5" brands={brands} />
        </div>
        <div className='ml-5 col-span-3'>
          ...
          <div className='grid grid-cols-4 gap-y-4'>
            {_subcategory.products.map((el: ProductItem, i: number) => (
              <ProductCard
                href={`/${slug}/${product_category}/${subcategory}/${el.id}`}
                key={i}
                id={el.id}
      ...
      {slug === "koshki" && product_category === "korm" && <div className='text-body-M'>
        <h3 className='text-header-XL text-black font-bold mb-1'>Корм для кошек: как выбирать, хранить и давать питомцу еду</h3>
        ...
        <h3 className='text-header-L text-black font-bold mt-3 mb-1'>Виды корма для котов: по типу, назначению и классу</h3>
        ...
        <h3 className='text-header-L text-black font-bold mt-3 mb-1'>Как выбрать подходящий корм для кошек?</h3>
        <ul className='font-normal text-gray-dark list-disc ml-10 mb-2'>
        ...
        <h3 className='text-header-L text-black font-bold mb-5'>Часто задаваемые вопросы</h3>
